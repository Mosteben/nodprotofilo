...
function MessageDialog({ message, onClose }: { message: MessageRow; onClose: () => void }) {
  ...
  const replyHref = message.email ? `mailto:${message.email}?subject=${encodeURIComponent(`رد: ${messageTitle(message, 60)}`)}` : null;
      ...
      <Dialog.Portal>
        ...
        >
          <div className="flex items-start justify-between gap-4 mb-4">
            <Dialog.Title className="font-display text-2xl text-navy leading-snug">{messageTitle(message)}</Dialog.Title>
          ...
          <dl className="grid sm:grid-cols-3 gap-3 rounded-xl bg-section p-4 font-ui text-sm mb-5">
            <div>
              ...
              <dd className="text-navy">{senderName(message)}</dd>
            </div>
            <div>
              ...
              >
                ...
                <span className="flex-1 min-w-0 font-ui">
                  <span className="flex items-center gap-2">
                    <span className={cn("text-sm truncate", m.is_read ? "text-navy/80" : "text-navy font-semibold")}>{messageTitle(m)}</span>
                    {!m.is_read && <StatusBadge status="unread" />}
                  </span>
                  <span className="block text-xs text-navy/50 truncate mt-0.5">
                    {senderName(m)}{m.email && <> · <span dir="ltr">{m.email}</span></>}
                  </span>
